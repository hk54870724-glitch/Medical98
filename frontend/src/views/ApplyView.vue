...
const submit=async()=>{ if(!form.yearId){Message.error('请选择报销年度',5000);return;} if(!form.applyDate){Message.error('请选择报销日期',5000);return;} for(let i=0;i<form.details.length;i++){const r=form.details[i];if(!r.invoiceName?.trim()){Message.error(`第${i+1}行缺少发票姓名`,5000);return;}if(!r.invoiceNo?.trim()){Message.error(`第${i+1}行缺少发票号码`,5000);return;}if(!r.invoiceDate){Message.error(`第${i+1}行缺少发票日期`,5000);return;}if(Number(r.selfPaid)<=0){Message.error(`第${i+1}行个人自付必须大于0`,5000);return;}if(!r.typeId){Message.error(`第${i+1}行请选择报销类型`,5000);return;}} submitting.value=true; try { const payload={yearId:form.yearId,applyDate:form.applyDate,details:form.details.map(r=>({invoiceName:r.invoiceName,invoiceNo:r.invoiceNo,invoiceDate:r.invoiceDate,totalAmount:Number(r.cashPaid||r.selfPaid||0),selfPaid:Number(r.selfPaid||0),typeId:r.typeId}))}; const r=await createApplication(payload); Message.success(`提交成功：${r.data.applicationNo}`); clearRows(); await refreshQuota(); } catch(e){Message.error(e.message,5000);} finally{submitting.value=false;} };
