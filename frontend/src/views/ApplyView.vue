        ...
        <div class="table-scroll">
          <a-table :data="form.details" :pagination="false" row-key="key">
            <template #columns>
              ...
              <a-table-column title="个人现金支付"><template #cell="{record}"><a-input-number v-model="record.selfPaid" :min="0" @change="calc" /></template></a-table-column>
...
const scanQr=async(file)=>{try{if(!('BarcodeDetector' in window))throw new Error('当前浏览器不支持二维码图片识别，请使用最新版Edge/Chrome或粘贴二维码解码文本');const detector=new BarcodeDetector({formats:['qr_code']});const bitmap=await createImageBitmap(file);const codes=await detector.detect(bitmap);bitmap.close();if(!codes.length)throw new Error('未识别到二维码，请上传清晰的发票二维码图片');const raw=codes[0].rawValue||'';if(!raw)throw new Error('二维码内容为空');if(/^https?:\/\//i.test(raw)){const r=await http.post('/parser/url-resolve',{url:raw});const parsed=r.data?.parsed;if(parsed&&parsed.invoiceNo){applyParsed(parsed);Message.success('网址票据已下载并解析，已填入首行');}else{Message.warning('二维码为网址，已下载文件但未能解析出发票信息，请手工核对');}}else{const r=await http.post('/parser/text',{text:raw,originalName:file.name});applyParsed(r.data);Message.success('发票二维码已识别并填入首行');}}catch(e){Message.error(e.message||'二维码识别失败',5000)}return false;};
const submit=async()=>{ if(!form.yearId){Message.error('请选择报销年度',5000);return;} if(!form.applyDate){Message.error('请选择报销日期',5000);return;} for(let i=0;i<form.details.length;i++){const r=form.details[i];if(!r.invoiceName?.trim()){Message.error(`第${i+1}行缺少发票姓名`,5000);return;}if(!r.invoiceNo?.trim()){Message.error(`第${i+1}行缺少发票号码`,5000);return;}if(!r.invoiceDate){Message.error(`第${i+1}行缺少发票日期`,5000);return;}if(Number(r.selfPaid)<=0){Message.error(`第${i+1}行个人现金支付必须大于0`,5000);return;}if(!r.typeId){Message.error(`第${i+1}行请选择报销类型`,5000);return;}} submitting.value=true; try { const payload={yearId:form.yearId,applyDate:form.applyDate,details:form.details.map(r=>({invoiceName:r.invoiceName,invoiceNo:r.invoiceNo,invoiceDate:r.invoiceDate,totalAmount:Number(r.selfPaid||0),selfPaid:Number(r.selfPaid||0),typeId:r.typeId}))}; const r=await createApplication(payload); Message.success(`提交成功：${r.data.applicationNo}`); clearRows(); await refreshQuota(); } catch(e){Message.error(e.message,5000);} finally{submitting.value=false;} };
onMounted(async()=>{setupAutoParse();try{ const c=await getContext();Object.assign(context,c.data); form.yearId=c.data.defaultYear.id; const r=await getCategories(); allTypes.value=r.data.flatMap(x=>x.types.map(t=>({...t,categoryName:x.name}))); addRow(); await refreshQuota(); }catch(e){Message.error(e.message,5000);}});
