    ...
    <div class="grid" style="gap:16px">
      ...
      <div class="page-card">
        <div class="toolbar">
          ...
          <a-checkbox v-model="autoParseEnabled" style="font-size:13px;color:#86909c">扫描枪自动解析</a-checkbox>
        ...
        <a-alert type="info" style="margin-bottom:12px">退休人员自动按退休人员分类；非退休人员依据发票姓名自动识别为职工本人或已登记子女。报销金额=个人自付×报销比例。</a-alert><div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px"><a-upload :auto-upload="false" :show-file-list="false" accept="image/*" @before-upload="scanQr"><a-button>识别发票二维码图片</a-button></a-upload><a-textarea v-model="parseText" placeholder="可粘贴发票文本或扫描枪输出（扫描后自动解析）" :auto-size="{minRows:2,maxRows:4}" style="min-width:260px;flex:1" @keydown.enter.prevent="onScannerEnter"/><a-button @click="parse">辅助解析并填入首行</a-button></div>
        <div class="table-scroll">
          <a-table :data="form.details" :pagination="false" row-key="key">
            <template #columns>
              ...
              <a-table-column title="个人现金支付"><template #cell="{record}"><a-input-number v-model="record.cashPaid" :min="0" /></template></a-table-column>
...
const allTypes=ref([]); const submitting=ref(false); const parseText=ref(''); const autoParseEnabled=ref(true); let autoParseTimer=null;
...
const calc=()=>{};
const applyParsed=(d)=>{if(!form.details.length)addRow();const row=form.details[0];if(d.invoiceNo)row.invoiceNo=d.invoiceNo;if(d.invoiceDate)row.invoiceDate=d.invoiceDate;if(d.payerName)row.invoiceName=d.payerName;const sp=Number(d.selfPaid);const ta=Number(d.totalAmount);if(Number.isFinite(sp)&&sp>0){row.cashPaid=sp;row.selfPaid=sp;}else if(Number.isFinite(ta)&&ta>0){row.cashPaid=ta;}};
const doParse=async(input)=>{if(!input){Message.warning('请先粘贴发票文本');return;} try{if(/^https?:\/\//i.test(input)){const r=await http.post('/parser/url-resolve',{url:input});const parsed=r.data?.parsed;if(parsed&&parsed.invoiceNo){applyParsed(parsed);Message.success('网址票据已解析，已填入首行');parseText.value='';}else{Message.warning('网址未解析出发票信息，请手工录入');}}else{const r=await http.post('/parser/text',{text:input});applyParsed(r.data);Message.success('解析结果已填入首行');parseText.value='';}}catch(e){Message.error({content:e.message,duration:5000})}};
const parse=()=>doParse(parseText.value.trim());
const onScannerEnter=()=>{parseText.value=parseText.value.trimEnd();parse();};
// 自动检测：输入稳定后（500ms无变化）且内容非空时自动触发
const setupAutoParse=()=>{watch(parseText,(val)=>{if(!autoParseEnabled.value)return;if(autoParseTimer)clearTimeout(autoParseTimer);const trimmed=val.trim();if(!trimmed)return;autoParseTimer=setTimeout(()=>{if(trimmed===parseText.value.trim()){doParse(trimmed);}},'500');});};
const scanQr=async(file)=>{try{if(!('BarcodeDetector' in window))throw new Error('当前浏览器不支持二维码图片识别，请使用最新版Edge/Chrome或粘贴二维码解码文本');const detector=new BarcodeDetector({formats:['qr_code']});const bitmap=await createImageBitmap(file);const codes=await detector.detect(bitmap);bitmap.close();if(!codes.length)throw new Error('未识别到二维码，请上传清晰的发票二维码图片');const raw=codes[0].rawValue||'';if(!raw)throw new Error('二维码内容为空');if(/^https?:\/\//i.test(raw)){const r=await http.post('/parser/url-resolve',{url:raw});const parsed=r.data?.parsed;if(parsed&&parsed.invoiceNo){applyParsed(parsed);Message.success('网址票据已下载并解析，已填入首行');}else{Message.warning('二维码为网址，已下载文件但未能解析出发票信息，请手工核对');}}else{const r=await http.post('/parser/text',{text:raw,originalName:file.name});applyParsed(r.data);Message.success('发票二维码已识别并填入首行');}}catch(e){Message.error({content:e.message||'二维码识别失败',duration:5000})}return false;};
const submit=async()=>{ if(!form.yearId){Message.error({content:'请选择报销年度',duration:5000});return;} if(!form.applyDate){Message.error({content:'请选择报销日期',duration:5000});return;} for(let i=0;i<form.details.length;i++){const r=form.details[i];if(!r.invoiceName?.trim()){Message.error({content:`第${i+1}行缺少发票姓名`,duration:5000});return;}if(!r.invoiceNo?.trim()){Message.error({content:`第${i+1}行缺少发票号码`,duration:5000});return;}if(!r.invoiceDate){Message.error({content:`第${i+1}行缺少发票日期`,duration:5000});return;}if(Number(r.selfPaid)<=0){Message.error({content:`第${i+1}行个人自付必须大于0`,duration:5000});return;}if(!r.typeId){Message.error({content:`第${i+1}行请选择报销类型`,duration:5000});return;}} submitting.value=true; try { const payload={yearId:form.yearId,applyDate:form.applyDate,details:form.details.map(r=>({invoiceName:r.invoiceName,invoiceNo:r.invoiceNo,invoiceDate:r.invoiceDate,totalAmount:Number(r.cashPaid||r.selfPaid||0),selfPaid:Number(r.selfPaid||0),typeId:r.typeId}))}; const r=await createApplication(payload); Message.success(`提交成功：${r.data.applicationNo}`); clearRows(); await refreshQuota(); } catch(e){Message.error({content:e.message,duration:5000});} finally{submitting.value=false;} };
onMounted(async()=>{setupAutoParse();try{ const c=await getContext();Object.assign(context,c.data); form.yearId=c.data.defaultYear.id; const r=await getCategories(); allTypes.value=r.data.flatMap(x=>x.types.map(t=>({...t,categoryName:x.name}))); addRow(); await refreshQuota(); }catch(e){Message.error({content:e.message,duration:5000});}});
onUnmounted(()=>{if(autoParseTimer)clearTimeout(autoParseTimer);});
</script>
