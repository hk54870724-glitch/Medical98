...
const context = reactive({availableYears:[],employee:null});
...
const scanQr=async(file)=>{try{if(!('BarcodeDetector' in window))throw new Error('当前浏览器不支持二维码图片识别，请使用最新版Edge/Chrome或粘贴二维码解码文本');const detector=new BarcodeDetector({formats:['qr_code']});const bitmap=await createImageBitmap(file);const codes=await detector.detect(bitmap);bitmap.close();if(!codes.length)throw new Error('未识别到二维码，请上传清晰的发票二维码图片');const raw=codes[0].rawValue||'';if(!raw)throw new Error('二维码内容为空');const r=await http.post('/parser/text',{text:raw,originalName:file.name});applyParsed(r.data);Message.success('发票二维码已识别并填入首行');}catch(e){Message.error(e.message||'二维码识别失败')}return false;}; const parse=async()=>{if(!parseText.value.trim()){Message.warning('请先粘贴发票文本');return;} try{const r=await http.post('/parser/text',{text:parseText.value});applyParsed(r.data);Message.success('解析结果已填入首行');}catch(e){Message.error(e.message)}};
