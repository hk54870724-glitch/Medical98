...
export async function updateEmployee(user,id,p){const cur=(await query(`SELECT enabled FROM employees WHERE id=$1`,[id])).rows[0];if(!cur)throw new AppError('EMPLOYEE_NOT_FOUND','员工不存在',404);const enabled=p.enabled!==undefined?p.enabled:cur.enabled;const r=await query(`UPDATE employees SET employee_no=$1,name=$2,gender=$3,department=$4,hire_date=$5,leave_date=$6,employment_status=$7,enabled=$8,updated_at=NOW() WHERE id=$9 RETURNING *`,[p.employeeNo,p.name,p.gender,p.department||null,p.hireDate,p.leaveDate||null,p.employmentStatus,enabled,id]);return r.rows[0];}
export async function deleteEmployee(user,id){return withTransaction(async client=>{const r=await client.query(`UPDATE employees SET enabled=false,employment_status=CASE WHEN employment_status='ACTIVE' THEN 'RESIGNED' ELSE employment_status END,updated_at=NOW() WHERE id=$1 RETURNING *`,[id]);if(!r.rowCount)throw new AppError('EMPLOYEE_NOT_FOUND','员工不存在',404);await client.query(`UPDATE employee_children SET enabled=false,updated_at=NOW() WHERE employee_id=$1 AND enabled=true`,[id]);return r.rows[0];});}
export async function enableEmployee(user,id){const r=await query(`UPDATE employees SET enabled=true,employment_status=CASE WHEN employment_status='RESIGNED' THEN 'ACTIVE' ELSE employment_status END,updated_at=NOW() WHERE id=$1 RETURNING *`,[id]);if(!r.rowCount)throw new AppError('EMPLOYEE_NOT_FOUND','员工不存在',404);return r.rows[0];}
export async function listChildren(employeeId){return (await query(`SELECT * FROM employee_children WHERE employee_id=$1 ORDER BY enabled DESC,id`,[employeeId])).rows;}
...
export async function updateChild(id,p){const cur=(await query(`SELECT enabled FROM employee_children WHERE id=$1`,[id])).rows[0];if(!cur)throw new AppError('CHILD_NOT_FOUND','子女不存在',404);const enabled=p.enabled!==undefined?p.enabled:cur.enabled;const r=await query(`UPDATE employee_children SET child_name=$1,gender=$2,birth_date=$3,enabled=$4,disabled_date=CASE WHEN $4 THEN NULL ELSE disabled_date END,updated_at=NOW() WHERE id=$5 RETURNING *`,[p.childName,p.gender||null,p.birthDate||null,enabled,id]);return r.rows[0];}
export async function deleteChild(id,p={}){const raw=String(p.disabledDate||'').trim();const disabledDate=/^\d{4}-\d{2}-\d{2}$/.test(raw)?raw:null;const r=await query(`UPDATE employee_children SET enabled=false,disabled_date=$2,updated_at=NOW() WHERE id=$1 RETURNING *`,[id,disabledDate]);if(!r.rowCount)throw new AppError('CHILD_NOT_FOUND','子女不存在',404);return r.rows[0];}
export async function enableChild(id){const r=await query(`UPDATE employee_children SET enabled=true,disabled_date=NULL,updated_at=NOW() WHERE id=$1 RETURNING *`,[id]);if(!r.rowCount)throw new AppError('CHILD_NOT_FOUND','子女不存在',404);return r.rows[0];}
...
export async function updateUser(user,id,p){
  ...
  if(!['EMPLOYEE','HR','FINANCE','ADMIN'].includes(roleCode)) throw new AppError('ROLE_INVALID','请选择有效的用户角色',422);
  if(roleCode==='EMPLOYEE'){ if(!Number.isInteger(employeeId)||employeeId<=0) throw new AppError('EMPLOYEE_BINDING_REQUIRED','员工角色必须绑定员工主数据，请选择对应员工',422); const er=await query(`SELECT 1 FROM employees WHERE id=$1 AND enabled=true`,[employeeId]); if(!er.rowCount) throw new AppError('EMPLOYEE_BINDING_INVALID','绑定的员工主数据不存在或已停用',422); }
  const cur=(await query(`SELECT enabled FROM users WHERE id=$1`,[id])).rows[0];if(!cur)throw new AppError('USER_NOT_FOUND','账号不存在',404);
  const enabled=p.enabled!==undefined?p.enabled:cur.enabled;
  const r=await query(`UPDATE users SET role_code=$1,employee_id=$2,enabled=$3,updated_at=NOW() WHERE id=$4 RETURNING id,username,role_code,employee_id,enabled,must_change_password`,[roleCode,roleCode==='EMPLOYEE'?employeeId:null,enabled,id]);return r.rows[0];
}
